...
public class Tower_Bomb : MonoBehaviour
{
    ...
    float m_AttackSpeed = 6.0f;

    //타워 UpGrade관련 변수
    bool m_UpgradeOnOff = false; //업그레이드OnOff
    [HideInInspector] public int m_TowerValue = 180;   //타워 값어치
    int m_AtSpeedUpIdx = 0;   //공속 업그레이드 횟수를 담을 인덱스
    int m_AtSpeedUpGold = 160; //공석 업그레이드 횟수에 따른 필요 골드값
    int m_AtPowerUpIdx = 0;   //공격력 업그레이드 횟수를 담을 인덱스
    int m_AtPowerUpGold = 160; //공석 업그레이드 횟수에 따른 필요 골드값
    int m_AtRangeUpIdx = 0;   //공격범위 업그레이드 횟수를 담을 인덱스
    int m_AtRangeUpGold = 100; //공격범위 업그레이드 횟수에 따른 필요 골드값
    //타워 UpGrade관련 변수
    ...
    void Start()
    {
        ...
        m_AttackSpeed = 6.0f;
        BombCtrl.m_MvSpeed = 0.7f;
        BombCtrl.m_Damage = 40;
        //이전게임에서 업그레이드한 변수들 초기화
    ...
    void ThrowBomb()
    {
        ...
        {
            GameObject a_ArrowObj = Instantiate(m_BombPrefab) as GameObject;
            a_ArrowObj.transform.position = m_Shootpos.position;

            Sound_Mgr.Instance.PlayEffSound("cannon_01", 0.4f);
        }
    }
    ...
    void AtSpeedUpBtnClick()
    {
        ...
        //BombCtrl.m_MvSpeed -= 0.5f;             //폭탄이속증가
        ...
        Sound_Mgr.Instance.PlayGUISound("UIClick1", 1.0f);
    }
    ...
    void SellBtnClick()
    {
        ...
        SlotScript a_Slot = a_TowerInstallMgr.m_TowerSlots[1].GetComponent<SlotScript>();
